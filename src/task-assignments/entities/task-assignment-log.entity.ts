...
export class TaskAssignmentLog {
    ...
    @Column({
        name: 'changed_by_family_member_id',
        type: 'varchar',
        length: 36,
        nullable: true,
    })
    changedByFamilyMemberId!: string | null;

    @ManyToOne(() => FamilyMember, (familyMember) => familyMember.assignmentLogs, {
        nullable: true,
        onDelete: 'SET NULL',
    })
    @JoinColumn({ name: 'changed_by_family_member_id' })
    changedByFamilyMember!: FamilyMember | null;
